...
# Load environment variables (Telegram Tokens, etc.)
load_dotenv()

# Synchronous MongoDB connection for tools
...
def search_trains(source_code: str, destination_code: str, date: str) -> str:
    ...
    try:
        ...
        if "data" in raw_data and "trainList" in raw_data["data"]:
            # 🚨 NAYA LOGIC: Current Date aur Time nikal rahe hain
            current_date_str = datetime.now().strftime("%d-%m-%Y")
            current_time_str = datetime.now().strftime("%H:%M")
            
            valid_trains = []
            
            # Step 1: Trains ko filter karo
            for train in raw_data["data"]["trainList"]:
                departure_time = train.get("departureTime", "00:00")
                
                # Agar journey aaj ki hai, aur train ka time nikal chuka hai, toh usko skip (continue) kardo
                if date == current_date_str and departure_time < current_time_str:
                    continue
                    
                valid_trains.append(train)
            
            # Step 2: Bachi hui valid trains me se max 8 uthao ([:8] error-free hai chahe list kitni bhi chhoti ho)
            for train in valid_trains[:8]: 
                ...
                for coach_class, details in avail_cache.items():
                    # 🚨 PERMANENT FIX: Extract directly and explicitly handle None
                    ...
                    availability_info[coach_class] = {
                        "status": raw_status if raw_status is not None else "Chart Prepared / N/A",
                        "fare": raw_fare if raw_fare is not None else "N/A",
                        "prediction": raw_prediction if raw_prediction is not None else "" 
                    }
                simplified_trains.append({
                    "Train_Name": train.get("trainName", "Unknown"),
                    "Train_Number": train.get("trainNumber", train.get("trainNo", "Unknown")),
            ...
            return json.dumps({"status": "success", "type": "trains", "data": simplified_trains})
            
        return json.dumps({"status": "no_trains_found", "message": "No trains found in the parsed data."})
...
def search_flights(source_airport: str, destination_airport: str, date: str) -> str:
    ...
    try:
        ...
        if "data" in raw_data and "itineraries" in raw_data["data"]:
            ...
            raw_flights_list = itineraries.get("topFlights", []) + itineraries.get("otherFlights", [])
            
            from datetime import datetime
            current_time = datetime.now()
            
            # SLICE HATA DIYA: Ab hum saari flights scan karenge
            for item in raw_flights_list: 
                # SMART LIMIT: 8 flights milte hi loop break kar do
                if len(simplified_flights) >= 8:
                    break
                    
                flight_segments = item.get("flights", [])
                if not flight_segments: continue
                ...
                # ---------------------------------------------------------
                # 🕒 THE TIME SHIELD: Past flights filter karna
                # ---------------------------------------------------------
                try:
                    # 'date' variable (YYYY-MM-DD) aur time ko jodd kar datetime object banayenge
                    # API generally "08:30 AM" ya "14:30" format deti hai
                    time_string = dep_time_raw.strip()
                    flight_dt_str = f"{date} {time_string}"
                    
                    try:
                        # Format check 1: 12-hour (AM/PM)
                        flight_dt = datetime.strptime(flight_dt_str, "%Y-%m-%d %I:%M %p")
                    except ValueError:
                        # Format check 2: 24-hour
                        flight_dt = datetime.strptime(flight_dt_str, "%Y-%m-%d %H:%M")
                        
                    # Agar flight ka time nikal chuka hai, toh yeh iteration skip kar do!
                    if flight_dt < current_time:
                        continue 
                except Exception as e:
                    # Agar api ne ajeeb format diya, toh filter na karke safety ke liye allow kar do
                    pass
                
                # Clean up timings for display
                dep_time = dep_time_raw[-8:].strip() if len(dep_time_raw) >= 8 else dep_time_raw
                arr_time = arr_time_raw[-8:].strip() if len(arr_time_raw) >= 8 else arr_time_raw
                
                layovers = item.get("layovers")
                prediction_text = f"{len(layovers)} Stop(s)" if layovers else "Non-stop Flight"
                
                simplified_flights.append({
                    "Train_Name": first_segment.get("airline", "Unknown Airline"), # Using Train_Name key so UI doesn't break
            ...
            return json.dumps({"status": "success", "type": "flights", "data": simplified_flights})
            
        return json.dumps({"status": "no_flights_found", "message": "Tell user no future flights found for today."})
        
    except Exception as e:
        return json.dumps({"status": "error", "message": f"API Error: {str(e)}"})

@tool
def find_accommodations(destination_address: str, stay_days: float, estimated_distance_km: float, purpose: str, pnr: str = "CAB9998887"):
    """ALWAYS use this tool when the user reaches their destination and needs to book a stay."""
    response_data = {"recommended_type": "HOTELS", "reasoning": f"Since you are staying for {stay_days} days, here are the best nearby hotels with real-time availability.", "options": []}
    ...
    try:
        ...
        for prop in properties[:4]: # Fetch top 4
            # 1. Image Extraction Fix (Use original_image)
            images = prop.get("images", [])
            img_url = images[0].get("original_image") if images else ""
            if not img_url:
                img_url = images[0].get("thumbnail", "") if images else "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800&q=80"
            ...
            dynamic_rooms = [
                ...
                {"type": "Deluxe AC Room", "price": int(base_price * 1.3), "status": "Fast Filling"},
                {"type": "Premium Suite", "price": int(base_price * 1.8), "status": "Few Left"}
            ]

            response_data["options"].append({
                "id": f"HTL-{prop.get('property_token', random.randint(1000,9999))}",
                "name": prop.get("name", "Local Hotel"),
                "description": prop.get("description", "Premium city accommodation."),
                "price": base_price,
                "image": img_url,
                "rating": prop.get("overall_rating", 4.0),
                "distance": "Near city center",
                "amenities": prop.get("amenities", ["Free Wi-Fi", "Room Service", "Power Backup", "AC"])[:5],
                "rooms": dynamic_rooms
            })
    except Exception as e:
        print(f"Hotel API Error: {e}")
        
    return json.dumps({"status": "success", "type": "accommodations", "data": response_data})


@tool
def create_hotel_draft(hotel_name: str, room_type: str, price: int, pnr: str = "") -> str:
    """Call this tool ONLY when the user LOCKS a hotel room. MUST output [HIDDEN_HOTEL_DRAFT: {draft_id}]"""
    
    # 🔥 THE BULLETPROOF PNR FIX (Using Trip Phase)
    if not pnr or pnr == "CAB9998887":
        # Sirf us trip ko uthao jo destination par pahunch chuki hai aur hotel dhoondh rahi hai
    ...
    checkout_link = f"http://localhost:5173/hotel-checkout/{draft_id}"
    ...
    if token and chat_id:
        ...
        try:
            requests.post(f"https://api.telegram.org/bot{token}/sendMessage", json={"chat_id": chat_id, "text": tg_message})
        except Exception:
            pass

    return json.dumps({
        "status": "success", "type": "hotel_draft", "draft_id": draft_id, "checkout_url": checkout_link,
        "hotel_name": hotel_name, "room_type": room_type, "price": price, "pnr": pnr
    })
...
def predict_train_delay(train_name: str) -> str:
    ...
    if any(keyword in name_upper for keyword in ["VANDE BHARAT", "RAJDHANI", "SHATABDI", "DURONTO", "TEJAS"]):
        ...
        reason = "Standard Express/Superfast train. Subject to regular route congestion."
        reliability = "75%"
    ...
    return json.dumps({"status": "success", "type": "train_delay_prediction", "data": analysis_data})
...
def get_destination_intel(city: str, travel_date: str) -> str:
    ...
    return json.dumps({"status": "success", "type": "destination_intel", "data": analysis_data})
...
@tool
def create_draft_booking(transit_details: str, passengers_json: str, fare_total: str) -> str:
    """Call this tool to create a DRAFT booking and send link to Telegram."""
    draft_id = f"DRAFT_{uuid.uuid4().hex[:6].upper()}"
    checkout_link = f"http://localhost:5173/checkout/{draft_id}"
...
import requests
from datetime import datetime, timedelta
# Make sure sync_db and @tool are imported/available as they were in your original file

@tool
def send_booking_confirmation(pnr: str, journey_details: str, passengers_raw_json: str,train_number: str,flight_number: str = "N/A", travel_class: str = "2A", fare: float = 1500.0, dep_time: str = "00:00", arr_time: str = "00:00", booking_status: str = "CNF", travel_date: str = "") -> str:
    """
    CRITICAL DIRECTIVE FOR AGENT: 
    1. You MUST execute this tool to save the booking to MongoDB.
    2. 'passengers_raw_json' MUST be the EXACT raw JSON array string from the SYSTEM prompt. DO NOT summarize or format it. Pass it exactly as received.
    3. 'travel_date' MUST be inferred exactly from the user's context (YYYY-MM-DD).
    
    4. 🚨 E-TICKET OUTPUT DIRECTIVE: After calling this tool, you MUST HALT all cross-selling. Do NOT ask about cabs, food, or hotels. You MUST reply to the user using EXACTLY one of the following Markdown formats depending on whether it is a Train or a Flight, and then STOP:
    ...
    **Date:** [Travel Date]
    ...
    5. 🚨 TRANSPORT IDENTIFIER (TRAIN vs FLIGHT): 
    ...
    6. 🚨 STRICT TIME FORMAT DIRECTIVE (CRITICAL FOR FLIGHTS): 
    ...
    passengers_list = []
    try:
        # LLM will now pass the exact string: '[{"name": "Sneha", ...}]'
    ...
    for p in passengers_list:
        ...
        if "WL" in booking_status.upper() or "RAC" in booking_status.upper() or "REGRET" in booking_status.upper():
            berth_info = booking_status
        else:
            # ✈️ FLIGHT VS 🚂 TRAIN SEAT LOGIC
            if flight_number != "N/A":
                # Flights ke liye: Row (2-32) aur Seat (A-F) e.g., 12A, 14F
                row = random.randint(2, 32)
                seat_letter = random.choice(['A', 'B', 'C', 'D', 'E', 'F'])
                berth_info = f"{row}{seat_letter}"
            else:
                # Trains ke liye: Coach (A/B/H/S) aur Seat (1-72)
                coach_prefix = "B" if "3A" in travel_class else "A" if "2A" in travel_class else "H" if "1A" in travel_class else "S"
                berth_info = f"{coach_prefix}{random.randint(1,6)}, Seat {random.randint(1,72)}"
            
        allocated_details.append(f"👤 {name} ({age}, {gender}) - Class: {travel_class} | Status: {berth_info}")
    ...
    if train_number != "N/A":
        # Pass the extracted details to our helper function
        train_origin_date = calculate_origin_date(train_number, journey_details, travel_date)
    ...
    booking_payload = {
        ...
        "is_solo": is_solo,
        "passengers": passengers_list,   # Will correctly save the JSON array now!
        "seats": parsed_seats,           # Will correctly show Berth or WL now!
        "total_amount": fare,
        "expenses": [{
            "item_id": f"TKT_{pnr}",
            "expense_type": "TRAIN_TICKET" if train_number != "N/A" else "FLIGHT_TICKET",
            "description": journey_details,
            "cost": fare,
            "status": "BOOKED",
            "timestamp": datetime.now().isoformat()
        }],
        "status": booking_status if "Paid" in booking_status or "PAID" in booking_status else f"Paid ({booking_status})",
        "timestamp": datetime.now().isoformat()
    }
    
    try:
        sync_db.bookings.insert_one(booking_payload)
        print(f"✅ DB Update: Saved {len(passengers_list)} passengers for PNR {pnr}. Origin Date: {train_origin_date}")
    except Exception as e:
    ...
    if token and chat_id:
        if flight_number != "N/A":
            import random
            # Thoda real feel dene ke liye ek random Terminal add kar dete hain 
            # (Agar API hoti toh hum asli terminal lete, but mock ke liye it's fine)
