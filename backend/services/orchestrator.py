...
def trigger_pre_departure_workflow(chat_id: str, pnr: str):
    ...
    seat_only = (
        seats_info.split("Status: ")[-1] if "Status: " in seats_info else seats_info
    )
    ...
    if is_flight:
        ...
        origin_date = latest_booking.get(
            "train_origin_date", datetime.datetime.now().strftime("%Y%m%d")
        )
        ...
        try:
            ...
            if live_data and "body" in live_data:
                ...
                current_station = data_body.get(
                    "train_status_message", "En route to your station"
                )
                ...
                if " from " in journey_details and " to " in journey_details:
                    boarding_code = (
                        journey_details.split(" from ")[1].split(" to ")[0].strip()
                    )
                ...
                for stn in stations_list:
                    ...
                    if (
                        boarding_code
                        and stn.get("stationCode", "").upper() == boarding_code.upper()
                    ):
                        boarding_eta = stn.get(
                            "actual_departure_time", stn.get("departureTime", "")
                        )
        ...
        keyboard = {
            "inline_keyboard": [
                [
                    {
                        "text": "🚕 Book Cab to Station",
                        "callback_data": f"book_cab_{pnr}",
                    }
                ],
                [{"text": "No, I'll manage myself", "callback_data": "ignore_cab"}],
            ]
        ...
        food_keyboard = {
            "inline_keyboard": [
                [
                    {
                        "text": "🍛 Order Masala Dosa",
                        "callback_data": f"food_usual_{pnr}",
                    }
                ],
                [
                    {
                        "text": "🍕 Order Something Else",
                        "callback_data": f"food_custom_{pnr}",
                    }
                ],
                [{"text": "No, I'm good", "callback_data": "food_no"}],
            ]
    ...
    return {
        "status": "success",
        "message": f"Pre-departure workflow executed for PNR {pnr}.",
    }
...
def trigger_wake_up_call(
    chat_id: str, pnr: str, destination: str = "Prayagraj Junction"
):
            ...
            [
                {
                    ...
                    "callback_data": "transit_cab",
                }
            ],
            [
                {
                    "text": "🎒 Light Luggage (Book Rapido Bike)",
                    "callback_data": "transit_bike",
                }
            ],
...
def trigger_project_shakti_onboarding(chat_id: str, pnr: str, passenger_name: str):
    ...
    # 1. Attach the Persistent Global SOS Keyboard
    persistent_keyboard = {
        "keyboard": [[{"text": "🚨 EMERGENCY SOS"}]],
    ...
    requests.post(
        ...
        json={
            "chat_id": chat_id,
            "text": "🛡️ Emergency protocols loaded.",
    ...
    inline_keyboard = {
        "inline_keyboard": [
            [
                {
                    "text": "🛡️ Activate Shakti Shield",
                    "callback_data": f"activate_shakti_{pnr}",
                }
            ]
        ]
    }
