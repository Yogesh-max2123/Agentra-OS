...
# --- IMPORTING THE NEW SCHEMA WITH EXPENSES ---
...
# ---------------------------------------------------------
# NEW SERVICES (Hum inko next step me banayenge)
# ---------------------------------------------------------
# from services.invoice_generator import generate_trip_invoice
# from services.ai_summary import generate_ai_invoice_summary

# ---------------------------------------------------------
# 🤖 THE MASTER AUTOPILOT ENGINE
# ---------------------------------------------------------
def master_timetable_checker():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🤖 Autopilot: Checking upcoming journeys...")
    
    try:
        # 🚨 FIX: Matches any status that contains "Paid" (e.g., "Paid (Confirmed)", "Confirmed & Paid")
        ...
        for booking in bookings:
            ...
            # Remove Z or timezone info to make it naive like datetime.now()
            ...
            print(f"   [Debug] {transport_icon} PNR {pnr} | T-Minus: {diff_minutes:.1f} mins | Status: {booking.get('status')}")
            ...
            if is_flight:
                ...
                if 60 <= diff_minutes <= 240:
                    if not booking.get('notified_pre_departure'):
                        ...
                        db.bookings.update_one({"_id": booking["_id"]}, {"$set": {"notified_pre_departure": True}})
            else:
            ...
            if not is_flight:
                ...
                if -25 <= diff_minutes <= -15:
                    if not booking.get('notified_post_boarding'):
                        ...
                        if gender == 'FEMALE' and is_solo:
                            ...
                            trigger_project_shakti_onboarding(chat_id, pnr, passenger_name)
                        
                        db.bookings.update_one({"_id": booking["_id"]}, {"$set": {"notified_post_boarding": True}})
                ...
                if diff_minutes < -60:
                    ...
                    if hours_passed > last_update_hour:
                        ...
                        if 'arrival_time' in booking:
                            ...
                            arr_diff_minutes_temp = (arr_time - current_time).total_seconds() / 60
                            
                        if arr_diff_minutes_temp > 60:
                            # 🚂 NEW DIRECT DB FETCH FOR HOURLY LOOP
                            ...
                            raw_dep_f = booking['departure_time'].replace("Z", "").split("+")[0]
                            f_date = datetime.fromisoformat(raw_dep_f).strftime("%Y%m%d")
                            
                            print(f"-> Firing Hourly Update ({hours_passed} Hour) for {pnr}")
                            
                            current_delay = trigger_hourly_live_tracking(chat_id, pnr, t_num, f_date)
                            
                            db.bookings.update_one(
                                {"_id": booking["_id"]}, 
                                {"$set": {
                                    "last_hourly_update_hour": hours_passed,
                                    "current_delay_minutes": current_delay if current_delay else 0
                                }}
                            )
                ...
                if 'arrival_time' in booking:
                    ...
                    arr_diff_minutes = (effective_arr_time - current_time).total_seconds() / 60
                ...
                if 0 < arr_diff_minutes <= 30 and not booking.get('notified_arrival'):
                    ...
                    print(f"-> Firing Smart Wake-up for {pnr} (Delay: {booking.get('current_delay_minutes', 0)} mins)")
                    
                    journey_str = booking.get("journey", "")
                    dest_station = journey_str.split(" to ")[-1].strip() if " to " in journey_str else "your destination"
                    ...
                    keyboard = {
                        "inline_keyboard": [
                            [{"text": "🚕 Yes, Book a Cab", "callback_data": f"transit_cab_{pnr}"}],
                            [{"text": "❌ No, I'm good", "callback_data": "transit_local"}]
                        ]
                    }
                    requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", json={"chat_id": chat_id, "text": msg, "reply_markup": keyboard, "parse_mode": "HTML"})
                    db.bookings.update_one({"_id": booking["_id"]}, {"$set": {"notified_arrival": True}})
                ...
                if 0 < arr_diff_minutes <= 10 and not booking.get('notified_cab'):
                    ...
                    keyboard = {
                        "inline_keyboard": [
                            [{"text": "🧳 Heavy Luggage (Book Ola Cab)", "callback_data": f"transit_cab_{pnr}"}],
                            [{"text": "🎒 Light Luggage (Book Rapido Bike)", "callback_data": f"transit_bike_{pnr}"}],
                            [{"text": "🚶 I will use Local Transit", "callback_data": "transit_local"}]
                        ]
                    }
                    requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", json={"chat_id": chat_id, "text": msg, "reply_markup": keyboard, "parse_mode": "HTML"})
                    db.bookings.update_one({"_id": booking["_id"]}, {"$set": {"notified_cab": True}})
            ...
            if is_flight:
                ...
                if -30 <= arr_diff_minutes <= 0 and not booking.get('notified_arrival'):
                    ...
                    journey_str = booking.get("journey", "")
                    dest_city = journey_str.split(" to ")[-1].split("(")[0].strip() if " to " in journey_str else "your destination"
                    p_name = booking.get("passenger_name", "Passenger")
                    ...
                    keyboard = {
                        "inline_keyboard": [
                            [{"text": "🏨 Proceed to Book Hotel", "callback_data": f"flight_hotel_{pnr}"}]
                        ]
                    }
...
async def create_new_booking(booking: BookingSchema):
    try:
        ...
        # 🚨 INVOICE: Calculate initial total_amount from expenses (train ticket)
        ...
        return {"status": "success", "message": "Booking Saved!", "id": str(result.inserted_id)}
    except Exception as e:
        return {"status": "error", "message": str(e)}

# ---------------------------------------------------------
# EVENT-DRIVEN ORCHESTRATION ENDPOINTS (Unchanged)
# ---------------------------------------------------------
...
from services.railway_api import get_live_train_status
# (Make sure send_real_sms is also imported or defined above this in main.py)



# 🚨 API-POWERED HOURLY LIVE TRACKING
def trigger_hourly_live_tracking(chat_id: str, pnr: str, train_number: str, formatted_date: str):
    bot_token = os.getenv("TELEGRAM_BOT_TOKEN")
    ...
    if live_data and "body" in live_data:
        ...
        if stations_list:
            ...
            final_eta = last_stn.get("actual_arrival_time", last_stn.get("arrivalTime", "N/A"))
    ...
    # 🚨 Yeh line important hai smart delay ke liye
    return delay_mins



# 🚨 DYNAMIC LIVE TRACKER ENGINE (ENRICHED PROJECT SHAKTI)
async def execute_shakti_live_tracker(chat_id: str, message_id: int, booking_data: dict):
    bot_token = os.getenv("TELEGRAM_BOT_TOKEN")
    base_url = f"https://api.telegram.org/bot{bot_token}/editMessageText"
    
    # Extracting exact details from the Database
    pnr = booking_data.get("pnr", "UNKNOWN") if booking_data else "UNKNOWN"
    p_name = booking_data.get("passenger_name", "Passenger") if booking_data else "Passenger"
    journey = booking_data.get("journey", "Unknown Route") if booking_data else "Unknown Route"
    seats = booking_data.get("seats", "Unassigned Seat") if booking_data else "Unassigned Seat"
    
    # 🚨 Extract Train Number and Name dynamically
    # 🚨 Extract Train Number dynamically (DB First, Regex as Fallback)
    ...
    # 🚨 NEW MASTER KEY FIX: Use exact Train Origin Date from DB
    formatted_date = booking_data.get("train_origin_date", datetime.now().strftime("%Y%m%d"))

    # 🚨 FETCHING REAL-TIME LIVE TRACKING DATA
    current_station = "En route (GPS locating...)"
    next_station = "Next Upcoming Station"
    eta_next = "Calculating..."
    speed = "Tracking Active" # Speed is not provided by this API directly, using fallback
    status_msg = "Live tracking active."
    ...
    if train_number and train_number != "Unknown":
        ...
        if live_data and "body" in live_data:
            ...
            # Rich status message direct from API (e.g. "Train has crossed Dingwahi...")
            status_msg = data_body.get("train_status_message", status_msg)
            
            current_station_code = data_body.get("current_station", "")
            stations_list = data_body.get("stations", [])
            
            # Find the current station and the next station in the JSON array
            for i, stn in enumerate(stations_list):
                if stn.get("stationCode") == current_station_code:
                    current_station = stn.get("stationName", current_station_code)
                    
                    # Look ahead for the next station
                    if i + 1 < len(stations_list):
                        next_stn = stations_list[i+1]
                        next_station = next_stn.get("stationName", "Next Station")
                        # Calculate ETA based on actual time if available, else scheduled time
                        eta_next = next_stn.get("actual_arrival_time", next_stn.get("arrivalTime", "N/A"))
                    break
    ...
    if message_id != 0:
        for i in range(20, 0, -5):
            ...
            keyboard = {
                "inline_keyboard": [
                    [{"text": "🚨 YES, I NEED HELP NOW", "callback_data": "shakti_confirm"}],
                    [{"text": "❌ False Alarm (Cancel)", "callback_data": "shakti_cancel"}]
                ]
            }
            requests.post(base_url, json={"chat_id": chat_id, "message_id": message_id, "text": countdown_msg, "parse_mode": "HTML", "reply_markup": keyboard})
    ...
    if emergency_chat_id:
        requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                      json={"chat_id": emergency_chat_id, "text": family_alert_msg, "parse_mode": "HTML"})
    
    # Send actual SMS to guardian
    ...
    # Ensure send_real_sms is imported and available in main.py
    try:
    ...
    if message_id != 0:
        ...
        requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                      json={"chat_id": chat_id, "text": "✅ Offline SOS processed successfully. RPF dispatched to next station.", "parse_mode": "HTML"})
# 🚨 THEFT PROTOCOL LIVE SIMULATION (Unchanged)
import re
import asyncio
import requests
import os
...
async def execute_theft_live_tracker(new_chat_id: str, message_id: int, booking_data: dict):
    bot_token = os.getenv("TELEGRAM_BOT_TOKEN")
    base_url = f"https://api.telegram.org/bot{bot_token}/editMessageText"
    ...
    # 🌐 ASLI SHAKTI API INTEGRATION (Dynamic Data)
    current_station = "En route (GPS locating...)"
    next_station = "Next Upcoming Station"
    eta_next = "Calculating..."
    status_msg = "Live tracking active."
    ...
    try:
        # Assuming get_live_train_status is globally available in your main.py
        if train_number and train_number != "Unknown":
            live_data = get_live_train_status(train_number, formatted_date)
    ...
    # 🚨 DYNAMIC ALERT FOR EMERGENCY CONTACT (Ab Orai nahi, asli data jayega)
    family_alert_msg = (
...
async def offline_sms_handler(
    ...
    if sms_text.startswith("SOS"):
        ...
        if len(parts) >= 2:
            pnr = parts[1] 
            booking = db.bookings.find_one({"pnr": pnr, "status": "Confirmed & Paid"}, sort=[("timestamp", -1)])
            
            if booking:
                chat_id = booking.get("chat_id")
                print(f"🚨 OFFLINE SOS TRIGGERED FOR PNR {pnr}")
                background_tasks.add_task(
                    execute_shakti_live_tracker, 
                    str(chat_id), 
                    0, 
                    booking
                )
...
async def telegram_webhook(request: Request):
    ...
    if "message" in data and "text" in data["message"]:
        ...
        if user_text.startswith("AGENTRA OVERRIDE"):
            ...
            if len(parts) == 3:
                override_pnr = parts[2].strip() # 🛡️ Hidden spaces remove karne ke liye
                
                print(f"-> EMERGENCY OVERRIDE INITIATED FOR PNR: '{override_pnr}'")
                
                # 🚨 THE FIX: Status filter hata diya. Emergency means Emergency!
                ...
                if stolen_booking:
                    ...
                    auth_keyboard = {
                        "inline_keyboard": [
                            [{"text": "🚨 MY PHONE IS STOLEN!", "callback_data": f"theft_confirm_{override_pnr}"}],
                            [{"text": "✅ I'm fine, just changing phones.", "callback_data": "theft_cancel"}]
                        ]
                    }
                    requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                                  json={"chat_id": chat_id, "text": auth_msg, "parse_mode": "HTML", "reply_markup": auth_keyboard})
                else:
                    print("-> ERROR: PNR Not Found in DB!")
                    requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                                  json={"chat_id": chat_id, "text": "❌ Override Failed: Invalid PNR or No Active Journey Found."})
            return {"status": "override_checked"}
        
        if "SHAKTI SOS" in user_text:
            shakti_states[chat_id] = "COUNTDOWN"
            
            # 🚨 THE SHIELD FIX: "flight_number": "N/A" ensure karega ki sirf TRAIN ticket uthe!
            user_booking = db.bookings.find_one(
                {
                    "chat_id": str(chat_id), 
                    "status": {"$regex": "Paid", "$options": "i"},
                    "flight_number": "N/A"  # <-- Yeh line flight ko block kar degi
                }, 
                sort=[("timestamp", -1)]
            )
            ...
            if not user_booking:
                requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                              json={"chat_id": chat_id, "text": "No active train journey found for SHAKTI SOS."})
                return {"status": "no_active_train"}
            
            initial_msg = "⚠️ <b>SHAKTI SHIELD INITIATED</b> ⚠️\nInitializing countdown..."
            response = requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                          json={"chat_id": chat_id, "text": initial_msg, "parse_mode": "HTML"})
            
            message_id = response.json()["result"]["message_id"]
            asyncio.create_task(execute_shakti_live_tracker(chat_id, message_id, user_booking))
            ...
            sos_keyboard = {
                "inline_keyboard": [
                    ...
                    [{"text": "🛑 Harassment / Misbehavior", "callback_data": "sos_harassment"}],
                    [{"text": "🧹 Coach Issue (Water/Cleaning)", "callback_data": "sos_cleaning"}]
                ]
            }
            requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                          json={"chat_id": chat_id, "text": sos_menu, "parse_mode": "HTML", "reply_markup": sos_keyboard})
            return {"status": "sos_menu_sent"}
        
        # 🚨 INVOICE: CAB CONFIRMED - Saving cost to DB
        if user_states.get(chat_id) and user_states[chat_id].startswith("waiting_for_address_"):
            ...
            requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                          json={"chat_id": chat_id, "text": booking_confirm_msg, "parse_mode": "HTML"})
            
            # Pushing expense to DB
            db.bookings.update_one(
                {"pnr": pnr},
                {
                    "$push": {"expenses": {"item_id": f"CAB_{random.randint(100,999)}", "expense_type": "CAB", "description": f"{ride_type} to {drop_address}", "cost": cab_cost, "status": "BOOKED", "timestamp": datetime.now()}},
        ...
        if user_states.get(chat_id) and str(user_states[chat_id]).startswith("waiting_for_food_"):
            pnr = user_states[chat_id].split("_")[3] # 🚨 STATE SE PNR NIKALA
            user_states[chat_id] = None
            ...
            requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                          json={"chat_id": chat_id, "text": food_confirm_msg, "parse_mode": "HTML"})
            
            # 🚨 STRICT PNR UPDATE
            db.bookings.update_one(
                {"pnr": pnr},
                {
                    "$push": {"expenses": {"item_id": f"FOOD_{random.randint(100,999)}", "expense_type": "FOOD", "description": user_text, "cost": food_cost, "status": "BOOKED", "timestamp": datetime.now().isoformat()}},
    ...
    if "callback_query" in data:
        ...
        # Keeping existing callback handlers intact...
        if callback_data.startswith("activate_shakti_"):
            ...
            requests.post(f"https://api.telegram.org/bot{bot_token}/editMessageText", 
                          json={"chat_id": chat_id, "message_id": message_id, "text": "✅ <b>Shakti Protocol Aborted.</b>\nStay safe! Let me know if you need anything else.", "parse_mode": "HTML"})
                          
        elif callback_data == "shakti_confirm":
            shakti_states[chat_id] = "CONFIRMED"
            requests.post(f"https://api.telegram.org/bot{bot_token}/answerCallbackQuery", 
                          json={"callback_query_id": callback_id, "text": "Escalating Immediately!"})
            ...
            requests.post(f"https://api.telegram.org/bot{bot_token}/editMessageText", 
                          json={"chat_id": chat_id, "message_id": message_id, "text": "✅ <b>Override Logged.</b> Happy to know you are safe. Your new device is now linked to your journey.", "parse_mode": "HTML"})
            ...
            requests.post(f"https://api.telegram.org/bot{bot_token}/answerCallbackQuery", 
                          json={"callback_query_id": callback_id, "text": "Initiating Anti-Theft Protocol!"})
            requests.post(f"https://api.telegram.org/bot{bot_token}/editMessageReplyMarkup", 
                          json={"chat_id": chat_id, "message_id": message_id, "reply_markup": {"inline_keyboard": []}})
                          
            asyncio.create_task(execute_theft_live_tracker(chat_id, message_id, stolen_booking))
            ...
            db.bookings.update_one(
                {"pnr": pnr},
                {
                    "$push": {"expenses": {"item_id": f"CAB_{random.randint(100,999)}", "expense_type": "CAB", "description": "Rapido/Ola Station Pickup", "cost": cab_cost, "status": "BOOKED", "timestamp": datetime.now().isoformat()}},
                    "$inc": {"total_amount": cab_cost}
                }
            )
             ...
             requests.post(f"https://api.telegram.org/bot{bot_token}/editMessageReplyMarkup", 
                          json={"chat_id": chat_id, "message_id": message_id, "reply_markup": {"inline_keyboard": []}})
             requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                          json={"chat_id": chat_id, "text": "Got it! Have a safe journey to the station."})
             
        
        
        elif callback_data.startswith("book_arrival_cab_"):
            pnr = callback_data.split("_")[3]
            requests.post(f"[https://api.telegram.org/bot](https://api.telegram.org/bot){bot_token}/editMessageReplyMarkup", json={"chat_id": chat_id, "message_id": message_id, "reply_markup": {"inline_keyboard": []}})
            ...
            requests.post(f"[https://api.telegram.org/bot](https://api.telegram.org/bot){bot_token}/sendMessage", json={"chat_id": chat_id, "text": confirmation_msg, "parse_mode": "HTML"})
            
            cab_cost = 250.0
            db.bookings.update_one(
                {"pnr": pnr},
                {
                    "$push": {"expenses": {"item_id": f"CAB_ARR_{random.randint(100,999)}", "expense_type": "LOCAL_TRANSIT", "description": "Pre-Arrival Station Pickup Cab", "cost": cab_cost, "status": "PAID", "timestamp": datetime.now().isoformat()}},
                    "$inc": {"total_amount": cab_cost}
                }
            )
            ...
            requests.post(f"[https://api.telegram.org/bot](https://api.telegram.org/bot){bot_token}/editMessageReplyMarkup", json={"chat_id": chat_id, "message_id": message_id, "reply_markup": {"inline_keyboard": []}})
            requests.post(f"[https://api.telegram.org/bot](https://api.telegram.org/bot){bot_token}/sendMessage", json={"chat_id": chat_id, "text": "Got it! Have a safe onward journey. You can check local transit options outside the station."})
        ...
        # ---------------------------------------------------------
            ...
            requests.post(f"https://api.telegram.org/bot{bot_token}/editMessageText", 
                          ...
                          json={"chat_id": chat_id, "message_id": message_id, "text": success_msg, "parse_mode": "HTML"})
        ...
        elif callback_data.startswith("transit_cab_") or callback_data.startswith("transit_bike_"):
            ...
            requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                          json={"chat_id": chat_id, "text": f"🗺️ Great! Where exactly do you want to go? (Please type your drop location for the {ride_type}):"})

        elif callback_data == "transit_local":
            requests.post(f"https://api.telegram.org/bot{bot_token}/editMessageReplyMarkup", 
                          json={"chat_id": chat_id, "message_id": message_id, "reply_markup": {"inline_keyboard": []}})
            requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                          json={"chat_id": chat_id, "text": "Got it! Have a safe onward journey. You can check local bus/metro routes outside the station."})
        ...
        # 🚨 INVOICE: USUAL FOOD (MASALA DOSA) - Saving cost to DB
            ...
            db.bookings.update_one(
                {"pnr": pnr},
                {
                    "$push": {"expenses": {"item_id": f"FOOD_{random.randint(100,999)}", "expense_type": "FOOD", "description": "Masala Dosa", "cost": dosa_cost, "status": "BOOKED", "timestamp": datetime.now().isoformat()}},
            ...
            requests.post(f"https://api.telegram.org/bot{bot_token}/editMessageReplyMarkup", 
                          json={"chat_id": chat_id, "message_id": message_id, "reply_markup": {"inline_keyboard": []}})
            # 🚨 STATE ME PNR DAAL DIYA
            user_states[chat_id] = f"waiting_for_food_{pnr}"
            force_reply_payload = {
                "chat_id": chat_id,
                "text": "🍽️ What would you like to order? (Type your food choice below):"
            }
            requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", json=force_reply_payload)

        elif callback_data == "food_no":
            requests.post(f"https://api.telegram.org/bot{bot_token}/editMessageReplyMarkup", 
                          json={"chat_id": chat_id, "message_id": message_id, "reply_markup": {"inline_keyboard": []}})
            requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                          json={"chat_id": chat_id, "text": "No worries! Have a safe and happy journey."})
            
        elif callback_data == "ack_settled":
            requests.post(f"https://api.telegram.org/bot{bot_token}/editMessageReplyMarkup", 
                          json={"chat_id": chat_id, "message_id": message_id, "reply_markup": {"inline_keyboard": []}})
            requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", 
                          json={"chat_id": chat_id, "text": "Awesome! Have a wonderful and safe journey. Agentra is always on guard. 🛡️"})
...
async def book_accommodation(req: BookStayRequest):
    ...
    # 🛡️ THE FINAL SHIELD: PNR Amnesia Fix 🛡️
    ...
    # Fetch existing booking using the CORRECTED PNR
    ...
    if booking:
        db.bookings.update_one(
            {"pnr": req.pnr}, 
            {
                "$set": {
                    ...
                    "status": "Completed",
                    "trip_phase": "CITY_EXPLORATION" # 🎯 CYCLE COMPLETE
                },
                "$push": {"expenses": {
        ...
        if updated_booking and chat_id:
            try:
                ...
                send_invoice_via_telegram(chat_id, pdf_path, updated_booking.get("total_amount", 0))
                print(f"✅ Invoice successfully sent to {chat_id} for PNR {req.pnr}")
            except Exception as e:
                print(f"❌ Error sending invoice: {e}")

    guest_names = ", ".join([p.get('name', 'Guest') for p in req.passengers]) if req.passengers else "Primary Guest"
    bot_token = os.getenv("TELEGRAM_BOT_TOKEN")
    ...
    return {
        ...
        "stay_booking_id": stay_booking_id, 
        "room_no": room_no,
        "pnr": req.pnr # <-- React UI ko asli PNR wapas de rahe hain!
    }
...
async def get_invoice_summary(pnr: str):
    try:
        ...
        # Simulating LLM Summary for now (hum AI next step me laga denge)
        total = trip.get("total_amount", 0)
        items = [f"{exp['expense_type']} (₹{exp['cost']})" for exp in trip.get("expenses", [])]
...
# ---------------------------------------------------------
# 🗺️ NEW ROUTE: SMART ITINERARY GENERATOR (GEMINI POWERED)
# ---------------------------------------------------------
import os
import json
...
# ---------------------------------------------------------
# 🗺️ SMART ITINERARY GENERATOR (100% PURE DYNAMIC FLOW)
# ---------------------------------------------------------
...
async def generate_smart_itinerary(pnr: str, req: ItineraryRequest):
    try:
        ...
        # 🚨 THE FIX: Strip Markdown forcefully if Gemini adds it
        ...
        # 4. SAVE ITINERARY DIRECTLY TO THIS SPECIFIC PNR (Using PNR instead of _id for extra safety)
        update_result = db.bookings.update_one(
            {"pnr": pnr}, 
            {"$set": {"itinerary_data": itinerary_json}}
        )
        
        print(f"✅ DB Update matched: {update_result.matched_count}, modified: {update_result.modified_count}")
...
async def get_saved_itinerary(pnr: str):
    try:
        # PURE DYNAMIC FETCH (No fallbacks needed anymore)
