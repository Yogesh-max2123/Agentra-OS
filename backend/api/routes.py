...
# 1. HELPER FUNCTION: Price History ke last 7 entries
...
async def process_chat(request: ChatRequest):
    ...
    logs = [
        {
            "time": current_time,
            "text": f"Received query: '{request.message}'",
            "type": "info",
        },
        {
            "time": current_time,
            "text": "Routing query with context history through LangGraph...",
            "type": "waiting",
        },
    ]

    try:
        # 2. Chat History Context build
        langchain_messages = []
        ...
        # 4. Extract Final Reply from LLM
        ...
        if price_summary and (
            "locked" in request.message.lower() or "price" in request.message.lower()
        ):
            ai_reply += f"\n\n[Stats: {price_summary}]"
        # 6. SMART CHIPS GENERATION
        chips = []
        ...
        logs.append(
            {
                ...
                "text": "Gemini tool orchestration complete.",
                "type": "success",
            }
        )
